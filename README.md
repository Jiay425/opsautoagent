...
定位 Agent 进入模型循环前，还会先构建 `preLoopCodeContextPack`。它由确定性仓库搜索和受限代码片段读取生成，再作为初始上下文交给 LLM。上下文包包含从告警抽出的搜索词、直接命中文件、候选代码片段、同包依赖、相关测试、构建/配置片段，以及每个文件进入上下文的原因。LLM 仍然可以继续调用工具验证或扩展，但第一轮不再是只靠告警文本盲猜。
