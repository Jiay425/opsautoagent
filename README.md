...
Before the model loop starts, the localization skill also builds a budgeted `preLoopCodeContextPack`. It is generated by deterministic repository search and bounded snippet reads, then passed to the LLM as initial context. The pack includes search terms extracted from the alert, directly hinted files, candidate source snippets, same-package dependencies, related tests, build/config snippets, and a reason for each file. A `budgetSummary` records included chars, truncated snippets, and remaining budget, so large repositories do not silently overflow the model context. The LLM may still call tools to verify or expand it, but the first turn is no longer a blind text-only guess.

After the model returns a localization result, a deterministic quality gate writes `localizationQuality` and `localizationReflection` into the trace. Low confidence, empty target files, missing supporting code evidence, or unresolved `missingEvidence` will trigger a reflection plan with `mustVerify`, suggested read-only tool calls, and optional `localizationBlocking` when code repair would be unsafe.

The eval harness now scores localization as a first-class stage, not only final patch success:

| Metric | Meaning |
|--------|---------|
| `Localization Target File Hit Rate` | Whether the Code Localization Agent found the expected root-cause file |
| `Localization Target Method Hit Rate` | Whether it found the expected root-cause method |
| `Localization Fix Strategy Accuracy` | Whether it chose code fix vs. config/resource/no-code action correctly |
| `Localization Scope Decision Accuracy` | Whether it chose single-method, multi-method, cross-file, or no-code scope correctly |
...
定位 Agent 进入模型循环前，还会先构建带预算控制的 `preLoopCodeContextPack`。它由确定性仓库搜索和受限代码片段读取生成，再作为初始上下文交给 LLM。上下文包包含从告警抽出的搜索词、直接命中文件、候选代码片段、同包依赖、相关测试、构建/配置片段，以及每个文件进入上下文的原因。`budgetSummary` 会记录实际放入的字符数、被截断的片段数和剩余预算，避免大仓库上下文静默撑爆模型窗口。LLM 仍然可以继续调用工具验证或扩展，但第一轮不再是只靠告警文本盲猜。

模型输出定位结果后，确定性质量门会把 `localizationQuality` 和 `localizationReflection` 写入 Trace。低置信度、目标文件为空、缺少代码证据或仍有 `missingEvidence` 时，会生成 `mustVerify`、建议的只读工具调用和下一轮约束；严重时会标记 `localizationBlocking`，避免证据不足却直接进入代码修复。

评测 Harness 也把代码定位单独作为一等指标，而不是只看最终补丁是否成功：

| 指标 | 含义 |
|------|------|
| `Localization Target File Hit Rate` | 定位 Agent 是否命中预期根因文件 |
| `Localization Target Method Hit Rate` | 是否命中预期根因方法 |
| `Localization Fix Strategy Accuracy` | 是否正确判断代码修复、配置/资源处理或不修代码 |
| `Localization Scope Decision Accuracy` | 是否正确判断单方法、多方法、跨文件或非代码修复范围 |
